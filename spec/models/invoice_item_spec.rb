...
RSpec.describe InvoiceItem, type: :model do
  ...
  describe "instance methods" do
    before(:each) do
      ...
      @bulk_discount3 = BulkDiscount.create!(percentage_discount: 30, quantity_threshold: 5, merchant_id: @merchant1.id)
      @bulk_discount2 = BulkDiscount.create!(percentage_discount: 25, quantity_threshold: 10, merchant_id: @merchant2.id)
      @bulk_discount4 = BulkDiscount.create!(percentage_discount: 20, quantity_threshold: 10, merchant_id: @merchant2.id)
    end
    describe "discount_applied" do
      it "returns the largest discount the invoice_item is eligible for" do
        expect(@ii_1.discount_applied).to eq(@bulk_discount1)
        expect(@ii_2.discount_applied).to eq(nil)
        expect(@ii_3.discount_applied).to eq(@bulk_discount2)
      end
    end
  end
end
