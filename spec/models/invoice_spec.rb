...
RSpec.describe Invoice, type: :model do
  ...
  describe "instance methods" do
    ...
    describe "discounted_revenue" do
      ...
      it "returns the discount amount for all items on an invoice" do
        ...
        expect(@invoice_1.discounted_revenue).to eq(95)
        expect(@invoice_2.discounted_revenue).to eq(55)
      end

      it "returns the total_revenue if there are no discounts" do
    ...
    describe "merchant_discounted_revenue" do
        ...
        it "returns discount on items on invoice for given merchant" do
          @bulk_discount1 = BulkDiscount.create!(percentage_discount: 50, quantity_threshold: 5, merchant_id: @merchant1.id)
          @bulk_discount2 = BulkDiscount.create!(percentage_discount: 25, quantity_threshold: 10, merchant_id: @merchant2.id)
          expect(@invoice_1.merchant_discounted_revenue(@merchant1)).to eq(45)
          expect(@invoice_1.merchant_discounted_revenue(@merchant2)).to eq(50)
        end
        it "considers each item's quantity when applying a discount, not combined item quantities" do
          ...
          expect(@invoice_1.merchant_discounted_revenue(@merchant1)).to eq(0)
        end
        it "applies the biggest discount if item quantity exceeds threshold for multiple discounts" do
          ...
          expect(@invoice_1.merchant_discounted_revenue(@merchant2)).to eq(100)
        end
        it "returns zero if there are no discounts" do
          expect(@invoice_1.merchant_discounted_revenue(@merchant1)).to eq(0)
          expect(@invoice_1.merchant_discounted_revenue(@merchant2)).to eq(0)
        end
      end
  end
    ...
    #   it "returns the total_revenue if there are no discounts" do
    #     expect(@invoice_1.all_discounted_revenue).to eq(300)
    #     expect(@invoice_2.all_discounted_revenue).to eq(300)
    #   end
    # end
